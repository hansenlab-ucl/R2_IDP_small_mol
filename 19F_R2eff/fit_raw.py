...
class fit_raw: 
    ...
    def fit_pseudo2D(self, dic=None, data_list=None, offset=-120):
    ...
    def r2(self, x=None, A=None, R2=None):
        return A*(np.exp(-R2*x))

    R2_delays = np.array([0.04, 0.28, 0.52, 0.76, 1.  , 1.2 , 0.04, 0.28, 0.52, 0.76, 1.  ,
       1.2 , 0.04, 0.28, 0.52, 0.76, 1.  , 1.2 ])

    def fit_R2_data(self, x=R2_delays, y=None):
        #p0 = (-20000, 20000, 0.2) #initial guess
        #bounds = ([-np.inf, 0, 0], [0, np.inf, np.inf])
        sort = np.argsort(x)
        popt, pcov = curve_fit(self.r2, x[sort], y[sort])
        R2 = popt[1]
        R2_error = np.sqrt(np.diag(pcov))[1]
        y_fit = self.r2(x[sort], *popt)
        plt.plot(x[sort], y[sort], 'bo', label='data')
        plt.plot(x[sort], y_fit, 'r-', label='fit')
        plt.ylim(0,1000)
        plt.xlabel('Time (s)')
        plt.ylabel('Intensity')
        plt.legend()
        plt.show()
        return popt, pcov, R2, R2_error
